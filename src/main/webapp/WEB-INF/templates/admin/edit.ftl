            ...
            <div class="box box-info">
                ...
                <form class="form-horizontal" action="${ctx}/admin/edit" method="post" id="myForm" role="form" data-toggle="validator">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label for="status" class="col-sm-2 control-label">投票状态</label>
                            <div class="col-sm-4">
                                <input type="text" class="form-control" id="status" name="status" value="${vote.status!}" placeholder="必填" required>
                                <div class="help-block with-errors"></div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4" id="options">
                                <#list optionList as option>
                                    <input type="text" class="form-control" id="${option.optionName!}" name="${option.optionName!}" value="${option.optionName!}" placeholder="必填">
                                    <div class="help-block with-errors"></div>
                                </#list>
                            </div>
                        </div>
...
<script>
    ...
    $('#sub').on('click', function () {
        ...
        var pastTime = new Date(Date.parse($('#pastTime').val()));
        var optionNameList = [];
        <#if optionList??>
            <#list optionList as option >
                optionNameList.push($('#' + '${option.optionName}').val());
            </#list>
        </#if>
        var dataObject = {
            id: id,
        ...
        httpPost("${ctx}/admin/edit", dataObject)
    });
