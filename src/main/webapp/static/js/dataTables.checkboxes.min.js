...
(function (window, document, undefined) {
    var factory = function ($, DataTable) {
        var Checkboxes = function (settings) {
            ...
            this.s = {dt: new DataTable.Api(settings), columns: [], data: {}, ignoreSelect: false};
        ...
        Checkboxes.prototype = {
            _constructor: function () {
                ...
                for (var i = 0; i < ctx.aoColumns.length; i++) {
                    if (ctx.aoColumns[i].checkboxes) {
                        ...
                        if (ctx.aoColumns[i].checkboxes.selectAll) {
                            $(dt.column(i).header()).html('<input type="checkbox">').addClass("dt-checkboxes-select-all").attr("data-col", i);
                        }
                    }
                }
                if (hasCheckboxes) {
                    ...
                    if (hasCheckboxesSelectRow) {
                        if (DataTable.select) {
                            ...
                            $tableContainer.on("click", "tbody td", function () {
                                ...
                                var e = {type: ($row.hasClass("selected") ? "deselect" : "select")};
                ...
                var cells = dt.cells("tr", col, {
                    page: ((ctx.aoColumns[col].checkboxes && ctx.aoColumns[col].checkboxes.selectAllPages) ? "all" : "current"),
                    search: "applied"
                });
                ...
                for (var colIdx = 0; colIdx < ctx.aoColumns.length; colIdx++) {
                    if (ctx.aoColumns[colIdx].checkboxes && ctx.aoColumns[colIdx].checkboxes.selectAll) {
                        ...
                        if ($checkboxesChecked.length === 0) {
                            $checkboxesSelectAll.prop({"checked": false, "indeterminate": false});
                        } else {
                            if ($checkboxesChecked.length === $checkboxes.length) {
                                $checkboxesSelectAll.prop({"checked": true, "indeterminate": false});
                            } else {
                                $checkboxesSelectAll.prop({"checked": true, "indeterminate": true});
                            }
                        }
                    }
                }
        ...
        Checkboxes.defaults = {selectRow: false, selectAll: true, selectAllPages: true};
